...
make_group(gp5,
	...
	/* 0x0f 0x01*/
make_group(gp7,
		EMPTY, EMPTY, EX(lgdt), EX(lidt),
		EX(jmp), EX(jmp), EMPTY, EMPTY)
	...
	opcode_entry opcode_table [512] = {
		...
		/* 0x20 */	IDEX(G2E, mov_Cd2Rd), EMPTY, IDEX(E2G, mov_Rd2Cd), EMPTY,
		...
		/* 0xb4 */	EMPTY, EMPTY, IDEX(mov_Eb2G,movzx), IDEX(mov_Ew2G,movzx),
		/* 0xb8 */	EMPTY, EMPTY, EMPTY, EMPTY,
		/* 0xbc */	EMPTY, EMPTY, IDEX(mov_Eb2G,movsx), IDEX(mov_Ew2G,movsx),
...
#define IRQ_TIMER 32
	...
	if (cpu.INTR && cpu.EFLAGS.IF) {
		...
		raise_intr((uint8_t)IRQ_TIMER, cpu.eip);
		update_eip();
	}
